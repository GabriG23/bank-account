...
function AccountStatus(props) {
  ...
  return (
    <>
      <Container fluid className="pt-5"> {/* rimosso mt-5 */}
        <Row className="min-vh-75">
          {/* Colonna 1/3: lista conti */}
          <Col md={4} className="p-4 border-end bg-light">
            <AccountList.AccountListAll tipo={"Lista di tutti i conti"} accounts={accountIDs} selectedAccountID={selectedAccountID} onSelect={handleSelect}/>
          </Col>

          {/* Colonna 2/3: dettagli */}
          <Col md={8} className="p-5 d-flex flex-column justify-content-center">
            ...
            {!loadingDetails && accountDetails && (
              <Card className="shadow-sm mx-auto"   style={{ maxWidth: '1000px', backgroundColor: accountDetails.account_status === 'OPEN' ? '#d1e7dd' : '#f8d7da'}}>
                <Card.Body className="fs-3 text-center">
            ...
            {!loadingDetails && !accountDetails && (
              <div className="text-muted text-center mt-5 fs-5">
